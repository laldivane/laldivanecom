...
export default function HomeContent({ homeData, latestRelease }: HomeContentProps) {
  // Determine Featured Content
  const featuredTitle = homeData?.featuredSignal?.title || latestRelease?.title || "CRIMSON FREQUENCIES";
  const featuredDesc = homeData?.featuredSignal?.description || latestRelease?.description || "The latest sonic transmission from the digital abyss.";
  const featuredImage = homeData?.featuredSignal?.customImage 
    ? urlForImage(homeData.featuredSignal.customImage).width(800).url()
    : latestRelease?.coverArt 
      ? urlForImage(latestRelease.coverArt).width(800).url()
      : "/assets/Crimson Frequencies.png";
  
  const featuredBadge = homeData?.featuredSignal?.badge || pageContent.home.featuredSignalBadge;
  
  const loreBadge = homeData?.loreSection?.badge || pageContent.home.loreBadge;
  const loreQuote = homeData?.loreSection?.quote || pageContent.home.loreQuote;
  const loreButton = homeData?.loreSection?.buttonText || pageContent.home.connectButton;
          ...
          >
            ...
            <div className="space-y-12 sm:space-y-16">
              <div className="space-y-6 sm:space-y-8">
                ...
                <h2 className="text-4xl sm:text-6xl md:text-7xl lg:text-8xl font-display font-black text-foreground leading-[0.85] tracking-tighter uppercase break-words">
                  {featuredTitle}
                </h2>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                {latestRelease?.platforms?.spotify && (
                   <a 
