...
// Define interface matching Sanity Track structure roughly, tailored for UI
export interface Track {
  id: string; // slug.current
  title: string;
  catalogId?: string;
  releaseDate: string;
  coverArt: any; // Sanity image object
  status: "draft" | "distributing" | "live";
  type: "single" | "album" | "ep";
  visualizerId?: string;
  platforms?: any;
  coverArtUrl?: string; // Pre-processed URL
}
...
interface DiscographyContentProps {
    tracks: Track[];
}

export default function DiscographyContent({ tracks: initialTracks }: DiscographyContentProps) {
  ...
  const stats = useMemo(() => {
    const total = initialTracks.length;
    const live = initialTracks.filter(t => t.status === 'live').length;
    const pending = initialTracks.filter(t => t.status !== 'live').length;
        ...
        <header className="mb-12 sm:mb-20">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
            ...
            <div className="flex items-center gap-8 sm:gap-12 p-6 rounded-2xl bg-white/[0.02] border border-white/5 backdrop-blur-sm">
              ...
              <div className="w-px h-8 bg-white/10" />
              <div className="text-center">
                <p className="text-2xl font-display font-black text-crimson">{stats.pending}</p>
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 mb-12">
          ...
          <div className="flex flex-wrap items-center gap-2">
            {(["all", "live", "distributing", "draft"] as const).map((status) => (
              ...
              >
                {status === "all" ? "All Transmissions" : STATUS_LABELS[status]?.label || status}
              </button>
            ))}
          </div>
                  ...
                  <div className="relative aspect-square rounded-lg sm:rounded-xl overflow-hidden border border-white/5 mb-3 group-hover:border-crimson/50 transition-all">
                    {track.coverArtUrl ? (
                        ...
                        <div className="w-full h-full bg-zinc-900 flex items-center justify-center">
                            <span className="material-symbols-outlined text-muted">music_note</span>
                        </div>
                    )}
                    
                    {/* Status Badge */}
                    <div className={`absolute top-2 left-2 px-2 py-0.5 rounded text-[8px] sm:text-[9px] font-bold uppercase tracking-wider border ${STATUS_LABELS[track.status]?.color || STATUS_LABELS.draft.color}`}>
                      {STATUS_LABELS[track.status]?.label || track.status}
                    </div>
                    {/* Visualizer Icon */}
                    {track.visualizerId && (
            ...
            {filteredTracks.map((track, idx) => (
              ...
              >
                ...
                >
                  ...
                  {/* Status */}
                  <div className={`hidden sm:block px-3 py-1 rounded-full text-[9px] font-bold uppercase tracking-wider border ${STATUS_LABELS[track.status]?.color || STATUS_LABELS.draft.color}`}>
                    {STATUS_LABELS[track.status]?.label || track.status}
                  </div>

                  {/* Platforms Count */}
                  <div className="text-xs text-muted hidden md:block">
                    {track.platforms ? Object.keys(track.platforms).length : 0} platform
                  </div>
